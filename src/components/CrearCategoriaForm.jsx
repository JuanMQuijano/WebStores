...
const CrearCategoriaForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!editando) {
                const { data } = await clienteAxio.post("/categorias", { name }, { headers: { 'x-token': localStorage.getItem("token-licorlab") } })

                setName("")

                setAlerta({ ok: data.ok, msg: data.msg })
            } else {
                const { data } = await clienteAxio.put(`/categorias/${id}`, { name }, { headers: { 'x-token': localStorage.getItem("token-licorlab") } })
